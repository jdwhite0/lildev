...
export class Vehicle {
	...
	update( dt, controlsInput ) {
		...
		if ( controlsInput.touchActive && ( this.inputX !== 0 || this.inputZ !== 0 ) ) {
			...
			this.linearSpeed = THREE.MathUtils.lerp( this.linearSpeed, MAX_SPEED * this.speedMul, dt * 1.5 * this.accelMul );
			...
			const targetAngular = - this.inputX * steeringGrip * 4 * this.gripMul * direction;
			...
			if ( targetSpeed < 0 && this.linearSpeed > 0.01 ) {
				...
				this.linearSpeed = THREE.MathUtils.lerp( this.linearSpeed, targetSpeed * MAX_SPEED * this.speedMul, dt * 1.5 * this.accelMul );
